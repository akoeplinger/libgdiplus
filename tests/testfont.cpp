...
static void readFile (const char *fileName, int *memoryLength, void **buffer)
{
	size_t length;
	size_t read_length;

	FILE *f = fopen (fileName, "rb");
	assertTrue (f) << "Expected file to exist.";
	...
	*buffer = malloc (length);
	assertTrue (*buffer) << "Expected successful allocation of buffer.";

	read_length = fread (*buffer, 1, length, f);
	assertTrue (read_length) << "Expected successful read.";
	fclose (f);
...
static void verifyFont (GpFont *font, GpFontFamily *originalFamily, INT expectedStyle, Unit expectedUnit)
{
	...
	assertTrue (font) << "Expected font to be initialized.";

	status = GdipGetFamily (font, &family);
	assertEqualInt (status, Ok);
	assertTrue (family && family != originalFamily);
	...
	assertEqualInt (status, Ok);
	assertTrue (!strcmp ((char *) originalFamilyName, (char *) familyName));
...
static void test_deletePrivateFontCollection ()
{
	...
	assertEqualInt (status, Ok);
	assertFalse (collection) << "Expected the pointer to be set to NULL.";
...
static void test_getFontCollectionFamilyList ()
{
	...
	assertEqualInt (status, Ok);
	assertEqualInt (numFound, 0);
	assertEqual (families[0], nullptr);
	assertEqual (families[1], nullptr);
...
static void test_privateAddFontFile ()
{
	...
	assertEqualInt (status, Ok);
	assertEqualString (name, "Code New Roman");
...
static void test_privateAddMemoryFont ()
{
	...
	GdipNewPrivateFontCollection (&collection);
	readFile ("test.ttf", &memoryLength, &memory);
	...
	// Valid OTF file.
	GdipNewPrivateFontCollection (&collection);
	readFile ("test.otf", &memoryLength, &memory);
...
static void test_newInstalledFontCollection ()
{
	...
	assertEqualInt (status, Ok);
	assertTrue (collection1) << "Expected collection to be initialized.";
	...
	assertEqualInt (status, Ok);
	assertEqual (collection1, collection2) << "Expected the FontCollecion to be a singleton.";

	status = GdipGetFontCollectionFamilyCount (collection2, &count2);
	assertEqualInt (status, Ok);
	assertEqual (count1, count2) << "Expected the FontCollection count to remain constant over multiple calls.";
}
...
static void test_cloneFont ()
{
	...
	assertEqualInt (status, Ok);
	assertTrue (clonedFont && font != clonedFont);
...
static void test_getLogfontA ()
{
	...
	assertEqualString (originalFamilyName, logfont.lfFaceName);
...
static void test_getLogfontW ()
{
	...
	assertEqualInt (logfont.lfQuality, 0);
	assertEqualInt (logfont.lfPitchAndFamily, 0);
	assertTrue (!strcmp ((char *) originalFamilyName, (char *) logfont.lfFaceName));
...
static void verifyFontFamily (GpFontFamily *family, const char *expectedName, UINT16 expectedCellAscent, UINT16 expectedCellDescent, UINT16 expectedLineSpacing, const char *alternateExpectedName, UINT16 alternateCellAscent, UINT16 alternateCellDescent, UINT16 alternateLineSpacing)
{
	...
	assertEqualInt (status, Ok);
	assertTrue (stringsEqual (name, expectedName) || stringsEqual (name, alternateExpectedName));
	...
	assertEqualInt (status, Ok);
	assertTrue (cellAscent == expectedCellAscent || cellAscent == alternateCellAscent);

	status = GdipGetCellDescent (family, FontStyleRegular, &cellDescent);
	assertEqualInt (status, Ok);
	assertTrue (cellDescent == expectedCellDescent || cellDescent == alternateCellDescent);

	status = GdipGetLineSpacing (family, FontStyleRegular, &lineSpacing);
	assertEqualInt (status, Ok);
	assertTrue (lineSpacing == expectedLineSpacing || lineSpacing == alternateLineSpacing);
...
static void test_getGenericFontFamilySansSerif ()
{
	...
	assertEqualInt (status, Ok);
	assertTrue (family2 && family1 == family2);
...
static void test_getFamilyName ()
{
	...
	assertEqualInt (status, Ok);
	assertTrue (stringsEqual (name, "Tahoma") || stringsEqual (name, "DejaVu Sans"));
...
LIBGDIPLUS_TEST (testfont, test_newPrivateFontCollection)
LIBGDIPLUS_TEST (testfont, test_deletePrivateFontCollection)
LIBGDIPLUS_TEST (testfont, test_getFontCollectionFamilyCount)
LIBGDIPLUS_TEST (testfont, test_getFontCollectionFamilyList)
LIBGDIPLUS_TEST (testfont, test_privateAddMemoryFont)
LIBGDIPLUS_TEST (testfont, test_privateAddFontFile)
LIBGDIPLUS_TEST (testfont, test_newInstalledFontCollection)
LIBGDIPLUS_TEST (testfont, test_createFontFromDC)
LIBGDIPLUS_TEST (testfont, test_createFontFromLogfontA)
LIBGDIPLUS_TEST (testfont, test_createFontFromLogfontW)
LIBGDIPLUS_TEST (testfont, test_createFont)
LIBGDIPLUS_TEST (testfont, test_cloneFont)
LIBGDIPLUS_TEST (testfont, test_deleteFont)
LIBGDIPLUS_TEST (testfont, test_getFamily)
LIBGDIPLUS_TEST (testfont, test_getFontStyle)
LIBGDIPLUS_TEST (testfont, test_getFontSize)
LIBGDIPLUS_TEST (testfont, test_getFontUnit)
LIBGDIPLUS_TEST (testfont, test_getFontHeight)
LIBGDIPLUS_TEST (testfont, test_getFontHeightGivenDPI)
LIBGDIPLUS_TEST (testfont, test_getLogfontA)
LIBGDIPLUS_TEST (testfont, test_getLogfontW)
LIBGDIPLUS_TEST (testfont, test_createFontFamilyFromName)
LIBGDIPLUS_TEST (testfont, test_cloneFontFamily)
LIBGDIPLUS_TEST (testfont, test_deleteFontFamily)
LIBGDIPLUS_TEST (testfont, test_getGenericFontFamilySansSerif)
LIBGDIPLUS_TEST (testfont, test_getGenericFontFamilySerif)
LIBGDIPLUS_TEST (testfont, test_getGenericFontFamilyMonospace)
LIBGDIPLUS_TEST (testfont, test_getFamilyName)
LIBGDIPLUS_TEST (testfont, test_isStyleAvailable)
LIBGDIPLUS_TEST (testfont, test_getEmHeight)
LIBGDIPLUS_TEST (testfont, test_getCellAscent)
LIBGDIPLUS_TEST (testfont, test_getCellDescent)
LIBGDIPLUS_TEST (testfont, test_getLineSpacing)
