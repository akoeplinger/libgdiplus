...
test_gdip_clip()
{
	...
	assertEqualInt (bm.Width, other_bm.Width);
	assertEqualInt (bm.Height, other_bm.Height);
	{
		ARGB *p = (ARGB *) bm.Scan0;
		ARGB *other_p = (ARGB *) other_bm.Scan0;
		for (int i = 0; i < width * height; ++i)
			assertEqual (*p++, *other_p++);
	}
...
test_gdip_clip_transform()
{
	...
	assertEqualInt (bounds.Y, 10);
	assertEqualInt (bounds.Width, 60);
	assertEqualInt (bounds.Height, 60);
...
LIBGDIPLUS_TEST (testclip, test_gdip_clip)
LIBGDIPLUS_TEST (testclip, test_gdip_clip_transform)
LIBGDIPLUS_TEST (testclip, test_gdip_clip_path)
