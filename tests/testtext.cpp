...
static void test_measure_string(void)
{
	...
	assertEqualInt (status, Ok);
	assertEqualFloat (5.0, bounds.X);
	assertEqualFloat (5.0, bounds.Y);
	assertEqualInt (glyphs, 1);
	assertEqualInt (lines, 1);
	...
	assertEqualInt (status, Ok);
	assertEqualInt (glyphs, 3);
	assertEqualInt (lines, 2);
	...
	assertEqualInt (status, Ok);
	assertEqualInt (glyphs, 4);
	assertEqualInt (lines, 3);
	...
	status = GdipMeasureString (graphics, teststring1, 2, font, &rect, format, &bounds, &glyphs, &lines);
	assertEqualInt (status, Ok);
	assertEqualInt (glyphs, 0);
	assertEqualInt (lines, 0);
	...
	assertEqualInt (status, Ok);
	assertEqualInt (glyphs, 3); // Should be reported despite being trimmed
	assertEqualInt (lines, 1);
	assertSimilarFloat(fontSize * 0.4, bounds.Width, fontSize * 0.3); // neither the expected value nor the precision is particularly accurate, but should be OK.
	...
	assertEqualInt (glyphs, 3); // Should be reported despite being trimmed
	assertEqualInt (lines, 1);
	assertEqualFloat ((double)fontSize, bounds.Width); // An em-space should be the same width as the font size.
...
static void test_measure_string_alignment(void)
{
	...
	for (i = 0; i < sizeof(td) / sizeof(td[0]); i++) {
		...
		assertEqualInt (status, Ok);
		assertSimilarFloat (td[i].x_x0 + td[i].x_xx * bounds.Width + 5.0, bounds.X, 0.6);
		assertSimilarFloat (td[i].y_y0 + td[i].y_yy * bounds.Height + 10.0, bounds.Y, 0.6);
		assertSimilarFloat (td[i].right_x0 + td[i].right_xx * bounds.Width + 5.0, bounds.X + bounds.Width, 0.6);
		assertSimilarFloat (td[i].bottom_y0 + td[i].bottom_yy * bounds.Height + 10.0, bounds.Y + bounds.Height, 0.6);
		...
		assertEqualInt (status, Ok);
		assertSimilarFloat (td[i].x_x0 + td[i].x_xx * bounds.Width + 5.0, bounds.X, 3.0);
		assertSimilarFloat (td[i].y_y0 + td[i].y_yy * bounds.Height + 10.0, bounds.Y, 3.0);
		assertSimilarFloat (td[i].right_x0 + td[i].right_xx * bounds.Width + 5.0, bounds.X + bounds.Width, 3.0);
		assertSimilarFloat (td[i].bottom_y0 + td[i].bottom_yy * bounds.Height + 10.0, bounds.Y + bounds.Height, 3.0);
	}
...
#if defined(USE_PANGO_RENDERING) || defined(USE_WINDOWS_GDIPLUS)
LIBGDIPLUS_TEST (testtext, test_measure_string)
#endif
LIBGDIPLUS_TEST (testtext, test_measure_string_alignment)
