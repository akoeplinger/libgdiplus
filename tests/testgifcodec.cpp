...
{ \
	...
	fclose (f); \
 \
	status = GdipLoadImageFromFile (wFile, &testgifcodec_image); \
...
{ \
  ...
  verifyBitmap (testgifcodec_image, gifRawFormat, PixelFormat8bppIndexed, 3, 5, ImageFlagsColorSpaceRGB | ImageFlagsHasRealDPI | ImageFlagsHasRealPixelSize | ImageFlagsReadOnly, 0, TRUE); \
  GdipDisposeImage (testgifcodec_image); \
}
...
LIBGDIPLUS_TEST (testgifcodec, test_validData)
LIBGDIPLUS_TEST (testgifcodec, test_invalidHeader)
LIBGDIPLUS_TEST (testgifcodec, test_invalidImageRecord)
LIBGDIPLUS_TEST (testgifcodec, test_invalidExtensionRecord)
